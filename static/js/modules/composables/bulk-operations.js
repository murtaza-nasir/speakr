...
export function useBulkOperations({
    ...
    const executeBulkReprocess = async () => {
        ...
        try {
            ...
            const isSummary = bulkReprocessType.value === 'summary';
            const baseKey = isSummary ? 'bulkReprocessModal.queuedSummaryToast' : 'bulkReprocessModal.queuedTranscriptionToast';
            const toastKey = count === 1 ? baseKey : baseKey + 'Plural';
            showToast(`${count} ${t(toastKey)}`, 'fa-sync-alt', 3000, 'success');
        } catch (error) {
