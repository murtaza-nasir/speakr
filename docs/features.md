...
Speakr detects it by sending a short fixed clip to the backend and comparing the embedding it gets back against the one recorded when voice matching first worked. The check runs when the transcription configuration changes, and on demand from the admin area. When it detects a change, Speakr reports it and keeps the original reference, so restoring the previous backend clears the warning by itself. If the change was intended, rebuild the affected voice profiles and then re-baseline to accept the new model. A backend that is simply unreachable is reported as a failed check rather than a changed model.
